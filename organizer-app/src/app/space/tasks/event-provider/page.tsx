...
export default async function EventProviderDocPage() {
  ...
  const docsPath = path.join(process.cwd(), 'docs', 'event-provider.md')
  let markdownContent = ''
  
  try {
    markdownContent = fs.readFileSync(docsPath, 'utf8')
  } catch (error) {
    console.error('Error reading Event Provider documentation:', error)
    markdownContent = '# Event Provider Documentation\n\nDocumentation not available. Please contact support.'
  }
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            ...
            <p className="text-slate-400">
              API для интеграции с внешними платформами управления событиями
            </p>
          </div>
        </div>
