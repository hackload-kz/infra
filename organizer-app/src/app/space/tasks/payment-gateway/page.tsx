...
export default async function PaymentGatewayDocPage() {
  ...
  const fileInfo = await getDocsFileInfo('payment-gateway.md')
  const docsPath = path.join(process.cwd(), 'public', 'docs', 'payment-gateway.md')
  let markdownContent = ''
  
  try {
    markdownContent = fs.readFileSync(docsPath, 'utf8')
  } catch (error) {
    console.error('Error reading payment gateway documentation:', error)
    markdownContent = '# Payment Gateway Documentation\n\nDocumentation not available. Please contact support.'
  }
