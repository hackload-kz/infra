...
### 1. Регистрация команды (Для администратора)
...
Здесь и везде далее, `teamSlug` указан как `MERCHANT_ID` в переменных окружения команды
...
**2. Покупатель отправляет данные карты** - `POST /api/v1/paymentform/submit`
